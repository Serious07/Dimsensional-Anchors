...
public class ContainerChunkLoader extends BaseContainer<TileChunkLoader> {
	...
	public void onButtonPressed(int i) {
		...
		if(i == 3) {
			...
			if(tile.getLoader().getOwner().equals(Owner.SERVER_OWNER_STRING))
				tile.getLoader().setOwner(Owner.getPlayerOwnerString(player.getGameProfile()));
			else
	...
	public void updateFields() {
		...
		canEdit = tile.getLoader().canEditBy(player, amIOp);
		amIOp = SidedProxy.instance.isOp(player);
