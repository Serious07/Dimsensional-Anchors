...
public class ChunkLoadInterface132 extends ChunkLoadInterface implements LoadingCallback {
	...
	static Method getOrCreateChunkWatcher;
	static Class<?> PlayerInstance;
	static {
		for(Method m : PlayerManager.class.getDeclaredMethods()) {
			Class<?>[] argtypes = m.getParameterTypes();
			if(argtypes.length == 3 && argtypes[0] == int.class && argtypes[1] == int.class && argtypes[2] == boolean.class) {
				if(getOrCreateChunkWatcher != null)
					throw new RuntimeException("More than one method matches getOrCreateChunkWatcher signature. First method: "+getOrCreateChunkWatcher+". Second method: "+m);
				getOrCreateChunkWatcher = m;
			}
		}
		if(getOrCreateChunkWatcher == null)
			throw new RuntimeException("failed to find getOrCreateChunkWatcher method");
		getOrCreateChunkWatcher.setAccessible(true);
		
		try {
			PlayerInstance = Class.forName("net.minecraft.server.management.PlayerManager$PlayerInstance");
	...
	public static boolean arePlayersWatchingChunk(PlayerManager pm, int x, int z) {
		try {
			Object chunk = getOrCreateChunkWatcher.invoke(pm, x, z, false);
			if(chunk == null)
				return false;
			
			return !((List)ReflectionHelper.getPrivateValue((Class)PlayerInstance, chunk, 0)).isEmpty();
