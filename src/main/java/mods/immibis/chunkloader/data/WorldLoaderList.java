...
public class WorldLoaderList extends WorldSavedData {
	...
	public void onPlayerLogin(UUID uuid) {
		onPlayerStateChange(uuid, true);
	}
	
	public void onPlayerLogout(UUID uuid) {
		onPlayerStateChange(uuid, false);
	}
	
	public void onPlayerLogin(String uuid) {
		onPlayerStateChange(UUID.fromString(uuid), true);
	}
	
	public void onPlayerLogout(String uuid) {
		onPlayerStateChange(UUID.fromString(uuid), false);
	}
	
	private void onPlayerStateChange(UUID uuid, boolean online) {
		for(Loader l : loaders)
			if(uuid.equals(Owner.getGameProfileID(l.owner)))
